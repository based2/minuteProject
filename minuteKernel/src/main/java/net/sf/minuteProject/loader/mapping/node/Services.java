...
public class Services extends AbstractConfigurationLoader{
   ...
   public Service[] getServicesArray() {
      return (Service[])getServices().toArray(new Service[getServices().size()]);
   }
   ...
   public void addService (Service _service) {
      getServices().add(_service);
   }
   
   public Service getFirstServiceFromServiceByName (String name) {
      if (name==null)
         return null;
      for (Service _service : getServices()) {
         if (_service.getName().equals(name))
            return _service;
      }
      return null;
   } 
